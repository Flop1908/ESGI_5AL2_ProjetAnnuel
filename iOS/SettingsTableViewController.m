...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	if(alertView.tag == 1 && buttonIndex == 1)
	{
		...
		NSString *url = @"mailto:support@ralf.org?subject=anne&doctique%20Fault";
	...
	if(alertView.tag == 2 && buttonIndex == 1)
	{
		...
		[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://ralf.org"]];
	}
}
...
- (IBAction)authorise:(id)sender
{
	...
	[prefs synchronize];
	redAPI = [(AppDelegate *) [[UIApplication sharedApplication] delegate] redditAPI];
...
{
	//To Change the user's Username or Nickname
...
//to Draw Splash Image On Button
...
//			break;
		...
		case SECTION_CONTACT:
			...
			if (indexPath.row == 2)
			{
				...
				[descLabel setText:@"Anne & Doctique is an open source project.  The source code is available from github.  We encourage developers to use the framework without restriction for non-commercial applications."];
...
//	}
	...
	if (indexPath.section == SECTION_CONTACT)
	{
		if (indexPath.row == 0)
		{
			NSString * message = @"This will launch your Mail application and exit Anne&Doctique.  Do you want to continue?";
		...
		{
			NSString * message = @"This will launch Mobile Safari and exit Anne&Doctique.  Do you want to continue?";
			UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Visit Ralf.org"
															message:message
														   delegate:self
												  cancelButtonTitle:@"No"
												  otherButtonTitles:@"Yes",nil];
