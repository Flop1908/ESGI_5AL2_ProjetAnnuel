...
static NSString * const AnneSegueRootIdentifier  = @"AnneAir_root";

@interface AnneAirViewController()
...
@implementation AnneAirViewController {
    ...
    // for animation
    BOOL            isAnimation;
    AnneSessionView * topSession;
    AnneSessionView * middleSession;
    AnneSessionView * bottomSession;
...
{
    [super viewDidLoad];
    NSLog(@"AirViewController Beginning");
    ...
    if ( self.storyboard) {
        @try {
            [self performSegueWithIdentifier:AnneSegueRootIdentifier sender:nil];
        }
    ...
    heightAirMenuRow = 44;
    NSLog(@"AirViewController finished");
}
...
- (void)prepareForSegue:(AnneAirViewControllerSegue *)segue sender:(id)sender
{
    if ( [segue isKindOfClass:[AnneAirViewControllerSegue class]] && sender == nil )
    {
        NSIndexPath * nextIndexPath = self.currentIndexPath;
        if ([segue.identifier isEqualToString:AnneSegueRootIdentifier]) {
        ...
        segue.performBlock = ^(AnneAirViewControllerSegue* rvc_segue, UIViewController* svc, UIViewController* dvc)
        {
...
- (void)reloadData
{
    NSLog(@"AirView's reload Begining");
    ...
    // Init AnneSessionView
    int sessionHeight = self.view.frame.size.height - kHeaderTitleHeight;
    for (int i = 0; i < session; i ++) {
        AnneSessionView * sessionView = sessionViews[@(i)];
        if (!sessionView) {
            sessionView = [[AnneSessionView alloc] initWithFrame:CGRectMake(30, 0, kSessionWidth, sessionHeight)];
    ...
    for (int i = 0; i < session; i ++) {
        AnneSessionView * sessionView = sessionViews[@(i)];
        // Remove all sub-view for contain of AnneSessionView
    ...
    // layout content view
    [self layoutContaintView];
    NSLog(@"AirView's reload finished");
}
...
- (void)layoutContaintView
{
    ...
    if (sessionViews.count == 1) {
        middleSession = sessionViews[@(0)];
        topSession = (AnneSessionView*)[self duplicate:middleSession];
        bottomSession = (AnneSessionView*)[self duplicate:middleSession];
    } else if(sessionViews.count == 2) {
        middleSession = sessionViews[@(currentIndexSession)];
        if (currentIndexSession == 0) {
            topSession = (AnneSessionView*)[self duplicate:sessionViews[@(1)]];
            bottomSession = sessionViews[@(1)];
        } else {
            topSession = (AnneSessionView*)[self duplicate:sessionViews[@(0)]];
...
#pragma mark - AnneAirMenuDelegate
...
@implementation UIViewController(AnneAirViewController)
...
- (UISwipeGestureRecognizer*)AnneSwipeGestureRecognizer{
...
- (void)setAnneSwipeHander:(void (^)(void))AnneSwipeHander
{
    if (AnneSwipeHander) {
        if (self.AnneSwipeGestureRecognizer.view) {
            [self.AnneSwipeGestureRecognizer.view removeGestureRecognizer:self.AnneSwipeGestureRecognizer];
        }
        
        if (self.navigationController) {
            [self.navigationController.view addGestureRecognizer:self.AnneSwipeGestureRecognizer];
        } else {
            [self.view addGestureRecognizer:self.AnneSwipeGestureRecognizer];
        }
        objc_setAssociatedObject(self, SwipeTagHandle, AnneSwipeHander, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        if (self.AnneSwipeGestureRecognizer.view) {
            [self.AnneSwipeGestureRecognizer.view removeGestureRecognizer:self.AnneSwipeGestureRecognizer];
        }
...
- (AnneAirViewController*)airViewController
{
    ...
    Class revealClass = [AnneAirViewController class];
...
- (void)dealloc
{
    self.AnneSwipeHander = nil;
}

@end


#pragma mark - AnneAirViewControllerSegue Class

@implementation AnneAirViewControllerSegue
